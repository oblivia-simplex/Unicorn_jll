...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import unicorn_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("unicorn_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "unicorn_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"unicorn", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "unicorn_jll", "src", "unicorn_jll.jl"))
        @info("unicorn_ll dev'ed out to /home/oblivia/.julia/dev/unicorn_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("213bd155-00d0-5096-9019-d950e7998be8"))
platforms = [Pkg.Artifacts.unpack_platform(e, "unicorn", artifacts_toml) for e in artifacts["unicorn"]]
...
if best_platform === nothing
    @debug("Unable to load unicorn; unsupported platform $(triplet(platform_key_abi()))")
else
